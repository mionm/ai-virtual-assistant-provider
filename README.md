...
#### Script-based quick start (service-only)

If you want a one-command Docker Compose flow (without notebooks), use the helper scripts:

```bash
cd $REPO_ROOT
cp .env.example .env
# Edit .env and set NVIDIA_API_KEY + NGC_API_KEY
chmod +x setup.sh start.sh stop.sh run.sh
./run.sh
```

Scripts summary:
- `setup.sh`: creates `.env` from `.env.example` if missing
- `start.sh`: logs in to `nvcr.io`, auto-selects free host ports, and starts the stack
- `stop.sh`: stops the stack
- `run.sh`: setup + start

Notebook note: this repo contains notebooks in `notebooks/` and `deploy/ai_virtual_assistant_notebook.ipynb`, but they are optional for this service-only flow.
