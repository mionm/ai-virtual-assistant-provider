...
class StatefulLogoutRequest(BaseModel):
    """Compatibility payload for stateful logout endpoints."""
    session_id: Optional[str] = Field(default="", description="Session id to close")
...
async def _best_effort_logout_session(session_id: str) -> dict:
    """
    End + delete a session if provided.
    This is best effort for UI compatibility and should not fail hard when session is absent.
    """
    if not session_id:
        return {"message": "No session_id provided; logout treated as no-op."}

    params = {"session_id": session_id}
    async with httpx.AsyncClient(timeout=httpx.Timeout(REQUEST_TIMEOUT)) as client:
        # Ignore non-200 failures here to keep logout idempotent and UI-friendly.
        try:
            await client.get(f"{AGENT_SERVER_URL}/end_session", params=params)
        except Exception as e:
            logger.warning(f"Best-effort end_session failed for {session_id}: {e}")
        try:
            await client.delete(f"{AGENT_SERVER_URL}/delete_session", params=params)
        except Exception as e:
            logger.warning(f"Best-effort delete_session failed for {session_id}: {e}")
    return {"message": f"Session logout handled for session_id={session_id}"}


@app.delete("/logout", tags=["Agent"])
@app.delete("/v2/predict/stateful/logout", tags=["Agent"])
@app.delete("/api/proxy/v2/predict/stateful/logout", tags=["Agent"])
async def stateful_logout(request: Request):
    """
    Compatibility endpoint for UI clients that call Watson-style stateful logout path.
    """
    ...
    if not session_id:
        try:
            ...
            if isinstance(payload, dict):
                session_id = payload.get("session_id", "") or payload.get("state", {}).get("session_id", "")
        except Exception:
            pass

    result = await _best_effort_logout_session(session_id)
    return result
